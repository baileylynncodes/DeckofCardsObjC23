...
#import "BMCardController.h"
#import "BMCard.h"

@implementation BMCardController

+ (instancetype)sharedController
{
    static BMCardController *sharedController = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^ {
        sharedController = [[BMCardController alloc] init];
    });
...
- (void)drawNewCard:(NSInteger)numberOfCards completion:(void (^)(NSArray<BMCard *> * _Nonnull, NSError * _Nonnull))completion
{
    NSString *cardCount = [@(numberOfCards) stringValue];
    NSURLComponents *urlComponents = [NSURLComponents componentsWithURL:[BMCardController baseURL] resolvingAgainstBaseURL: true];
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:searchURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        for (NSDictionary *cardDictionary in cardsArray) {
            ...
            BMCard *card = [[BMCard alloc] initwithDictionary:cardDictionary];
...
- (void)fetchCardImage:(BMCard *)card completion:(void (^)(UIImage * _Nonnull, NSError * _Nonnull))completion
{
